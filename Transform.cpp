...
// KIT307 - Graphics and Animation, 2018
// =========================================================================================
//
...
namespace T3D
{
	...
	//Increase Y rotation by positive amount
	void Transform::turnLeft(float degrees)
	{
		Vector3 objectRotation = this->getEulerAngles();
		Vector3 modifier = Vector3(0, oneDegree * degrees, 0);
		Vector3 finalRotation = objectRotation + modifier;
		rotationMatrix = Quaternion(finalRotation);
		needLocalUpdate = true;
		setNeedWorldUpdate();
	}
	//Turns right by supplying a negative amount to turn left
	void Transform::turnRight(float degrees) { turnLeft(-degrees); }

	//Increases X rotation by positive amount
	void Transform::lookUp(float degrees)
	{
		Vector3 objectRotation = this->getEulerAngles();
		Vector3 modifier = Vector3(oneDegree * degrees, 0, 0);
		Vector3 finalRotation = objectRotation + modifier;
		rotationMatrix = Quaternion(finalRotation);
		needLocalUpdate = true;
		setNeedWorldUpdate();
	}
	//Looks down by supplying a negative amount to look up
	void Transform::lookDown(float degrees) { lookUp(-degrees); }

	//Increases Z rotation by positive amount
	void Transform::rollLeft(float degrees)
	{
		Vector3 objectRotation = this->getEulerAngles();
		Vector3 modifier = Vector3(0, 0, oneDegree * degrees);
		Vector3 finalRotation = objectRotation + modifier;
		rotationMatrix = Quaternion(finalRotation);
		needLocalUpdate = true;
		setNeedWorldUpdate();
	}
	//Rolls right by providing a negative amount to roll left
	void Transform::rollRight(float degrees) { rollLeft(-degrees); }

	//Sets the rotation of the object in degrees ignoring previous rotation
	void Transform::setRotationInDegrees(float x, float y, float z)
	{
		Vector3 finalRotation = Vector3(oneDegree * x, oneDegree * y, oneDegree * z);
		rotationMatrix = Quaternion(finalRotation);
		needLocalUpdate = true;
		setNeedWorldUpdate();
	}
	//Functions to manipulate rotation. Can take positive or negative amounts.
	void Transform::turnY(float degrees) { turnLeft(degrees); }
	void Transform::turnX(float degrees) { lookUp(degrees); }
	void Transform::turnZ(float degrees) { rollLeft(degrees); }
